...
![Karel in a 10x5 grid at (6,1) facing east](resources/first_world.png)
The following block is source file for FirstWorldKarel.w, which lays out Karel's location and direction, beeper(s), and wall:
```
Dimension: (10, 5)
Wall: (5, 1) west
Beeper: (4, 1) 1
...
Speed: 1.50
```
*Task*: Get the beeper, then turn off.

*Question for world builders*: If you place at wall at, for instance, 3,1 east or 4,1 west, the world looks the same, but does Karel behave the same way when facing them?
...
Mountain climb
 ![Karel in a 10x6 grid at (2,1) facing east](resources/plant_flag.png)
Another from Pattis' first problem set.
Task: Climb the mountain, plant a flag (beeper) at the summit, descend in the east, pick up that beeper, turn off.
 {% spoiler Example %}
